...
class IdilEmployee(models.Model):
    ...
    pin = fields.Char(string='PIN', copy=False,
                      ...
                      tracking=True)
    image_1920 = fields.Image(string="Image", max_width=1920, max_height=1920, tracking=True)
    ...
    account_id = fields.Many2one('idil.chart.account', string='Commission Account',
                                 domain="[('account_type', 'like', 'commission'), ('code', 'like', '2%'), "
                                        "('currency_id', '=', currency_id)]", tracking=True
                                 )

    commission = fields.Float(string='Commission Percentage', tracking=True)

    # Salary and bonus information
    salary = fields.Monetary(string='Basic Salary', currency_field='currency_id', tracking=True)
    ...
    maker_checker = fields.Boolean(string='Maker & Checker', default=False, tracking=True)
    ...
    def write(self, vals):
        ...
        for record in self:
            hr_employee = self.env['hr.employee'].search([('idil_staff_id', '=', record.id)])
            if hr_employee:
                hr_employee.write({
