...
class VendorSummaryReportWizard(models.TransientModel):
    ...
    def generate_pdf_report(self):
        ...
        transaction_query = """
            SELECT 
                ROW_NUMBER() OVER (ORDER BY v.name) AS "Sl No",
                v.name AS "Vendor Name",
                COUNT(po.id) AS "# PO",
                SUM(po.amount) AS "Amount"
            FROM 
                idil_vendor_registration v
            INNER JOIN 
                idil_purchase_order po ON v.id = po.vendor_id
            WHERE 
                po.status = 'approved'
        ...
        # Table Header
        data = [["No", "Vendor Name", "# PO", "Balance"]]

        # Add transaction data
        for transaction in transactions:
            sl_no = transaction[0] or ""
            vendor_name = transaction[1] or "N/A"
            po_count = transaction[2] or 0
            total_amount = transaction[3] or 0.0

            grand_total_quantity += po_count
            grand_total_balance += total_amount
