...
class IdilEmployeeSalary(models.Model):
    ...
    deductions = fields.Monetary(string='Deductions', default=0.0, required=True,
                                 help="Deductions reduce the net salary.", tracking=True)
    advance_deduction = fields.Monetary(string='Advance Salary', default=0.0, store=True, tracking=True)

    total_salary = fields.Monetary(string='Total Salary', compute='_compute_total_salary', store=True, tracking=True)
    currency_id = fields.Many2one(
        'res.currency',
        string='Currency',
        readonly=True,
        default=lambda self: self.env.ref('base.USD').id, tracking=True
    )
    ...
    def _check_employee_salary_and_contract(self):
        ...
        for record in self:
            ...
            if record.employee_id.contract_start_date and record.employee_id.contract_end_date:
                if not (
                        record.employee_id.contract_start_date <= record.salary_date <= record.employee_id.contract_end_date):
                    raise ValidationError(
    ...
    def create(self, vals):
        ...
        if vals.get('employee_id') and vals.get('salary_date'):
            ...
            advances = self.env['idil.employee.salary.advance'].search([
                ...
                ('request_date', '<', (salary_date + relativedelta(months=1)).strftime('%Y-%m-%d'))  # End of the month
            ])
    ...
    def _book_transaction(self, record):
        ...
        transaction_lines = self.env['idil.transaction_bookingline'].search([
            ('account_number', '=', credit_account.id)  # Now credit_account is a recordset, so .id works fine
        ])
        current_balance = sum(transaction_lines.mapped('dr_amount')) - sum(transaction_lines.mapped('cr_amount'))
    ...
    @api.depends('basic_salary', 'bonus', 'deductions', 'advance_deduction')
    def _compute_total_salary(self):
        for record in self:
            record.total_salary = record.basic_salary + record.bonus - record.deductions - record.advance_deduction
        ...
        for record in self:
            if record.employee_id and record.salary_date:
                ...
                advances = self.env['idil.employee.salary.advance'].search([
                    ...
                    ('request_date', '<', (record.salary_date + relativedelta(months=1)).strftime('%Y-%m-%d'))
                ])
                record.advance_deduction = sum(advances.mapped('advance_amount'))
                advances.write({'state': 'deducted'})  # Mark advances as deducted
            else:
        ...
        for record in self:
            ...
            if 'employee_id' in vals or 'salary_date' in vals:
                ...
                advances = self.env['idil.employee.salary.advance'].search([
                    ...
                    ('state', '=', 'draft'),
                    ('request_date', '>=', year_month + '-01'),
                    ('request_date', '<', (salary_date + relativedelta(months=1)).strftime('%Y-%m-%d'))
                ])
        ...
        for employee in employees:
            if employee.contract_start_date and employee.contract_end_date:
                ...
                if employee.contract_start_date <= today <= employee.contract_end_date:
                    ...
                    salary_logs.append(f"Contract expired for employee {employee.name}.")
            else:
                salary_logs.append(f"Missing contract dates for employee {employee.name}.")
    ...
    def generate_salary_report_pdf(self, employee_id=None, export_type="pdf"):
        ...
        if export_type == "pdf":
            ...
            payment_table_data = [
                ...
                ["Basic Salary", f"${report_data['basic_salary']:,.2f}", "Deductions",
                 f"${report_data['deductions']:,.2f}"],
                ["Bonus", f"${report_data['bonus']:,.2f}", "Advance Deduction",
                 f"${report_data['advance_deduction']:,.2f}"],
                ["Gross Earnings", f"${report_data['basic_salary'] + report_data['bonus']:,.2f}", "Total Deductions",
            ...
            signature_table = [
                [f"Prepared by (HR): {hr_name}", "______________________", f"Received by (Employee): {employee_name}",
